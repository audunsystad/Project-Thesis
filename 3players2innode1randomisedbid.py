        ...
        for p1 in range(len(producer1_bids)):
            for p2 in range(len(producer2_bids)):
                if randomised_bid_selection_old or randomised_bid_selection_new:
                    rand = np.random.normal(0, c*1.818954026356906) # mean and stdev
        ...
        for p1 in range(len(producer1_bids)):
            for p2 in range(len(producer2_bids)):
                ...
                if bimatrix_volume[p1][p2][0]>0 and bimatrix_volume[p1][p2][2]>0: # redispatch for p1, p2, p3
                    ...
                    producer1_RTMP,producer3_RTMP = real_time_market_price,real_time_market_price
                    ...
                    if producer1_RTMP>producer3_RTMP:
                        redispatch_volume_p1 = min(bimatrix_volume[p1][p2][0],bimatrix_volume[p1][p2][0]+bimatrix_volume[p1][p2][2]+windmill_cap-transmission_cons)
                        redispatch_volume_p2 = (producer2_cap)
                        redispatch_volume_p3 = (bimatrix_volume[p1][p2][2]+windmill_cap-transmission_cons)
                    elif producer1_RTMP<producer3_RTMP:
                        redispatch_volume_p3 = min(bimatrix_volume[p1][p2][2],bimatrix_volume[p1][p2][0]+bimatrix_volume[p1][p2][2]+windmill_cap-transmission_cons)
                        redispatch_volume_p2 = producer2_cap
                    ...
                    if (p1,p2,p3) in tso_cost.keys():
                        frac_of_incdec[(p1,p2,p3)]+=1/num_runs
                        tso_cost[(p1,p2,p3)] += (redispatch_volume_p2*producer2_RTMP-redispatch_volume_p3*producer3_RTMP-redispatch_volume_p1*producer1_RTMP)/num_runs
                    else:
                        frac_of_incdec[(p1,p2,p3)]=1/num_runs
                        tso_cost[(p1,p2,p3)] = (redispatch_volume_p2*producer2_RTMP-redispatch_volume_p3*producer3_RTMP-redispatch_volume_p1*producer1_RTMP)/num_runs
                    ...
                    if (p1,p2,p3) in tso_cost.keys():
                        frac_of_incdec[(p1,p2,p3)]+=1/num_runs
                        tso_cost[(p1,p2,p3)] += total_redispatch_volume*(producer2_RTMP-producer1_bids[0])/num_runs
                    else:
                        frac_of_incdec[(p1,p2,p3)]=1/num_runs
                        tso_cost[(p1,p2,p3)] = total_redispatch_volume*(producer2_RTMP-producer1_bids[0])/num_runs
                    ...
                    inc_dec_gaming.append([p1,p2,p3])
                    redispatch_volume = bimatrix_volume[p1][p2][2]+windmill_cap-transmission_cons
                    p1_profit = 0
                    p2_profit = bimatrix_volume[p1][p2][1]*price+redispatch_volume*producer2_RTMP-producer2_cap*producer2_MC
                    ...
                    if (p1,p2,p3) in tso_cost.keys():
                        frac_of_incdec[(p1,p2,p3)]+=1/num_runs
                        tso_cost[(p1,p2,p3)] += total_redispatch_volume*(producer2_RTMP-producer3_bids[0])/num_runs
                    else:
                        frac_of_incdec[(p1,p2,p3)]=1/num_runs
                        tso_cost[(p1,p2,p3)] = total_redispatch_volume*(producer2_RTMP-producer3_bids[0])/num_runs
                else:
...
for result in NE_for1and2:
    inc_dec = "[No inc-dec]      "
    if producer1_bids[result[0]]<producer2_bids[result[1]] and producer2_bids[result[1]]>producer3_bids[result[2]]:
            inc_dec = "[Inc-dec apparent]"
    if result[2]==0: # NE_for1and2 given lowest p3 bid, i.e. table 0
        if lists_of_all_profits[0][result[0]][result[1]][2]>lists_of_all_profits[1][result[0]][result[1]][2]:
            ...
            frac_of_incdecs.append(frac_of_incdec[(result[0],result[1],result[2])])
            #print(frac_of_incdec[(result[0],result[1],result[2])])
            add_to_SPNE_dict((producer1_bids[result[0]],producer2_bids[result[1]],producer3_bids[result[2]]))
...
print("")
print("These are the averages over all runs")
...
#print(f"The fraction of SPNEs with inc-dec was {sum(frac_of_incdecs)/len(frac_of_incdecs)}")
